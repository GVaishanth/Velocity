...
window.TimingTable = (() => {
    ...
    function update() {
        ...
        if (window.VELOCITY_POSITION_DEBUG && typeof RaceEngine.getPositionDebugData === 'function') {
            const now = performance.now();
            if (now - lastPositionDebugLog > 2000) {
                lastPositionDebugLog = now;
                console.table(RaceEngine.getPositionDebugData(99));
            }
        }
    }
    ...
    function calculateGap(car) {
        ...
        // Same authoritative gap fields generated by RaceEngine.updatePositions()
        if ((car.gapToLeaderLaps || 0) > 0) {
            const lapDiff = car.gapToLeaderLaps;
            return `+${lapDiff} LAP${lapDiff > 1 ? 'S' : ''}`;
        }
        if (typeof car.gapToLeaderSeconds === 'number') return formatGap(car.gapToLeaderSeconds);

        // Defensive fallback mirrors RaceEngine's progress model.
        const leaderProgress = (leader.lapCount || 0) + (leader.trackProgress || 0);
        const carProgress = (car.lapCount || 0) + (car.trackProgress || 0);
        return formatGap(Math.max(0, (leaderProgress - carProgress) * (car.lastLapTime || leader.lastLapTime || 90)));
    }
    ...
    function calculateInterval(car) {
        ...
        // Same authoritative gap fields generated by RaceEngine.updatePositions()
        if ((car.gapToAheadLaps || 0) > 0) return `+${car.gapToAheadLaps}L`;
        if (typeof car.gapToAheadSeconds === 'number') return formatGap(car.gapToAheadSeconds);

        // Defensive fallback mirrors RaceEngine's progress model.
        const aheadProgress = (carAhead.lapCount || 0) + (carAhead.trackProgress || 0);
        const carProgress = (car.lapCount || 0) + (car.trackProgress || 0);
        return formatGap(Math.max(0, (aheadProgress - carProgress) * (car.lastLapTime || carAhead.lastLapTime || 90)));
    }
